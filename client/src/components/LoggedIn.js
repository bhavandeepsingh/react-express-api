...
export default function LoggedIn() {
  ...
  const fetchOrganizations = async () => {
    try {
      const accessToken = await getToken();
      const res = await fetch(`${process.env.REACT_APP_API_BASE_URL}/organizations`, {
        headers: {
          Authorization: `Bearer ${accessToken}`,
        },
      });
    ...
    <>
      ...
      <main>
        <div className="container">
          ...
          <section className="next-steps-section">
            <h2 className="text-heading-1">Next steps for you</h2>
            <button className="btn btn-dark" type="button" onClick={fetchOrganizations}>
              Fetch organizations
            </button>
            <ul>
              {organizations
                ? organizations.map((item) => <li key={item.code}>{item.name}</li>)
                : null}
            </ul>
          </section>
        </div>
